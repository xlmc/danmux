...
当前实现覆盖审批修订版任务书要求的标准骨架：双层版本、Base 校验、`gradient`（`texture` / `linear`）、B 站原生渐变适配、普通弹幕人工渐变、DanDanPlay fallback、Enhanced Extension、未知效果隔离、聚合幂等、Asset Resolver 安全边界、能力协商、结构化诊断和测试。
...
`p` 的字段顺序固定为 `time,mode,fontSize,color`；渐变不会被塞进 `p`。旧客户端无需修改即可消费 `p+m`，支持 DanmuX Gradient v1 的客户端只需在既有解析器之后读取 `danmux.effects`。
...
`fromBilibili()` 会把 B 站 `colorfulSrc` 的填充/描边纹理映射为 `origin=native` 的 gradient；格式异常时保留 Base，并生成 namespaced `vendor=bilibili` fallback。`applyGradient()` 只生成 `origin=generated` 的 linear gradient，preset 只是生成器配置，不会进入协议核心。原生效果优先于 generated 效果。
...
远程纹理必须通过 `AssetResolver`。默认限制包括：仅 HTTPS、拒绝 localhost/环回/内网/云 metadata 主机、拒绝重定向、限制图片 MIME、最大字节数、超时和可选 SHA-256 校验。下载失败时调用方必须继续使用 Base fallback。生产环境还应把解析器放在独立网络策略/缓存层，并继续增加像素数、解压后尺寸和内容扫描限制。
